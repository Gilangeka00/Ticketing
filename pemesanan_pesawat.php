...
// 3) Ambil detail tiket pesawat dari DB
$stmt = $pdo->prepare('SELECT * FROM tickets WHERE id = ? AND type = "pesawat"');
$stmt->execute([$ticket_id]);
$flight = $stmt->fetch(PDO::FETCH_ASSOC);
if (!$flight) {
    echo '<p class="text-red-500 text-center mt-10">Tiket pesawat tidak ditemukan.</p>';
    include 'footer.php';
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($posted_step === 1) {
        ...
        if ($availableSeats < 1) {
            $errors[] = 'Maaf, tiket pesawat ini sudah habis terjual.';
            $step = 1;
        }
        ...
        if (empty($errors)) {
            ...
            if ($currentBalance < $total_price) {
                ...
                try {
                    ...
                    $desc = "Pembelian {$quantity} tiket pesawat ({$flight['name']}). Kursi: " .
...
<div class="container mx-auto py-10">
  <div class="max-w-2xl mx-auto bg-white p-6 rounded-lg shadow-xl">
    <h2 class="text-3xl font-semibold mb-6 text-gray-800 border-b pb-4">Pemesanan Tiket Pesawat</h2>
    ...
    <?php if ($step === 1): ?>
      ...
      <?php if ($availableSeatsInfo < 1): ?>
        <div class="mb-6 bg-yellow-50 border-l-4 border-yellow-500 text-yellow-700 p-4 rounded-md" role="alert">
          <p>Tiket pesawat ini sudah habis terjual. Silakan kembali ke <a href="index.php" class="text-blue-600 hover:underline">Beranda</a>.</p>
        </div>
      <?php else: ?>
        <form method="POST" action="pemesanan_pesawat.php?id=<?= $ticket_id ?>">
    ...
    <?php if ($step === 2): ?>
      <div class="mb-6 p-4 bg-gray-50 border border-gray-200 rounded-lg">
        <p class="text-gray-700"><strong>Pesawat:</strong> <?= htmlspecialchars($flight['name']) ?></p>
        <p class="text-gray-700"><strong>Rute:</strong> <?= htmlspecialchars($flight['origin']) ?> &rarr; <?= htmlspecialchars($flight['destination']) ?></p>
